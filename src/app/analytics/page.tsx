...
export default function AnalyticsPage() {
  ...
  const isEarningsStats = activeSubMenu === 'Earning Statistics';
  const isEngagementMessages = activeSubMenu === 'Engagement / Messages' || !isEarningsStats;
  ...
  return (
    <div className="p-8 space-y-8 bg-zinc-950 text-white min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-900/60 pb-6">
        <div>
          ...
          <p className="text-zinc-500 text-xs font-semibold mt-1">
            {isEarningsStats 
              ? "Real-time tracked chatter shift performance reports and aggregated earnings statistics."
              : "Real-time subscriber cohort retention metrics and message/PPV conversion tracking logs."}
          </p>
        </div>
        <button
      ...
      {!activeCreator ? (
        <div className="border border-dashed border-zinc-800 rounded-3xl p-12 text-center text-zinc-550">
          <ShieldAlert className="h-10 w-10 text-zinc-650 mx-auto mb-4" />
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <>
                ...
                <div className="bg-zinc-900/40 border border-zinc-850 rounded-2xl p-6 relative overflow-hidden backdrop-blur-sm">
                  ...
                  <div className="mt-4 space-y-1">
                    ...
                    <p className="text-zinc-550 text-[10px] font-bold">
                ...
                <div className="bg-zinc-900/40 border border-zinc-850 rounded-2xl p-6 relative overflow-hidden backdrop-blur-sm">
                  ...
                  <div className="mt-4 space-y-1">
                    ...
                    <p className="text-zinc-550 text-[10px] font-bold">Average Month 4+ retention rate</p>
                  </div>
                </div>
              </>
            )}
            ...
            {isEarningsStats && (
              <>
                {/* Shift Logs Card */}
                <div className="bg-zinc-900/40 border border-zinc-850 rounded-2xl p-6 relative overflow-hidden backdrop-blur-sm col-span-3">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="mt-4 space-y-1">
                    ...
                    <p className="text-zinc-550 text-[10px] font-bold">Combined logged chatter shift revenues</p>
                  </div>
                </div>
              </>
            )}
                      ...
                      {cohortData.map((row) => (
                        <tr key={row.cohort} className="hover:bg-zinc-900/20 transition-colors">
                          ...
                          {row.retention.map((rate, idx) => {
                            let bgClass = "bg-zinc-950 text-zinc-650";
                            if (rate > 0) {
              ...
              <div className="bg-zinc-900/40 border border-zinc-850 rounded-2xl p-6 backdrop-blur-sm space-y-4">
                ...
                <div className="space-y-5 pt-2">
                  {conversionData?.priceTiers && conversionData.priceTiers.length > 0 ? (
                    ...
                    <div className="py-8 text-center text-zinc-555 text-xs italic">
                        ...
                        .map((op) => (
                          <tr key={op.id} className="hover:bg-zinc-900/20 transition-colors">
                            <td className="p-3">
                              ...
                              <span className="text-[10px] text-zinc-550 block">@{op.email.split('@')[0]}</span>
                            </td>
                            <td className="p-3">
